...
export function DowntimeParetoPage() {
  ...
  return (
    <div>
      ...
      {data && rows.length === 0 && (
        <p className="text-sm text-slate-400">No breakdown logs in this date range.</p>
      )}
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {withCumulative.map((row) => (
                  <tr key={row.reason_code ?? row.description} className="border-b border-slate-100 last:border-0">
                    <td className="px-4 py-2">
                      {row.description}
                      {row.category && <span className="ml-1 text-xs text-slate-400">({row.category})</span>}
                    </td>
                    <td className="px-4 py-2 text-right tabular-nums">{row.count}</td>
                    <td className="px-4 py-2 text-right tabular-nums">{row.total_minutes}</td>
