...
import type { FieldValue } from "@/src/components/DynamicField";

/** Client-side mirror of the backend's entries/services.py::coerce_value
 * validation — catches obvious mistakes immediately even with zero
 * connectivity, before the entry ever reaches the server for the
 * authoritative check at sync time.
 */
...
export function validateAll(
  parameters: FormSchemaParameter[],
  values: Record<string, FieldValue>,
): string[] {
  return parameters
    .map((p) => validateField(p, values[p.code]))
